...
pub struct AppState {
    ...
    pub player: IntMap<i32, EntityGroup>,
...
impl AppConfig {
    ...
    pub fn build(self, mut sub: QueueSub) -> (Arc<AppState>, DbConfig, QueueSub, ServerConfig) {
        let mut state = AppState {
            ...
            player: IntMap::with_capacity(self.players.len()),
        ...
        // Use separate subscriptions to avoid race conditions with JOINs
        // Include player_username_state for player names
        sub.push_query(move || vec![
            String::from("SELECT loc.* FROM mobile_entity_state loc;"),
            String::from("SELECT sip.* FROM signed_in_player_state sip;"),
            String::from("SELECT pus.* FROM player_username_state pus;"),
        ]);
